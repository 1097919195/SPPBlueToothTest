...
package com.example.sppbluetoothtest.library;
...
//                2.可解决蓝牙连接时发生socket已关闭或超时错误的方法 https://blog.csdn.net/vic_torsun/article/details/79650865
                ...
                Log.i("BLUETOOTHSERVICE", "misstopped:" + misstopped);
                if (misstopped == false)
                    connectionFailed();
                return;
            }
                ...
                try {
                    ...
                    if (print_mode) {//打印版的称
                        if (data == 0x28) {//协议5：28 6B 67 29 表示的字符是（kg），02开头，03结尾啊， 47 57 3A 表示的字符是GW：
                            if (arr_byte.size() > 0) {
                                buffer = new byte[arr_byte.size() - 8];//减去02开头和GW：  这里需要小心还有后面四位也要截取掉
                                for (int i = 0; i < arr_byte.size() - 8; i++) {
                                    buffer[i] = arr_byte.get(i + 8).byteValue(); //减去02开头和GW的字节所以获取发送的时候需要i+4
                                }
                    ...
                    Log.i("ConnectedThread", "isstopend:" + misstopped);
                    if (misstopped) {
                        break;
                    }
                    connectionLost();
